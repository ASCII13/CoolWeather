...
import com.example.genewong.coolweather.gson.Forecast;
import com.example.genewong.coolweather.gson.Weather;
import com.example.genewong.coolweather.util.HttpUtil;
import com.example.genewong.coolweather.util.Utility;
...
public class WeatherAct extends AppCompatActivity {
    ...
    private void showWeatherInfo(Weather weather) {
        ...
        for (Forecast forecast : weather.forecastList) {
            View view = LayoutInflater.from(this).inflate(R.layout.forecast_item_layout, forecastLayout, false);
    ...
    private void requestWeather(String weatherId) {
        ...
        String weatherUrl = "http://guolin.tech/api/weather?cityid="
                + weatherId
                + "&key=206b96f3ee044bdbb382d1983191a6cd";
        ...
        HttpUtil.sendRequestWithOkHttp(weatherUrl, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                final Weather weather = Utility.getWeatherResponse(responseText);

                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (weather != null && "ok".equals(weather.status)) {
                            SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(WeatherAct.this).edit();
        ...
        HttpUtil.sendRequestWithOkHttp(requestBingPic, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.with(WeatherAct.this).load(bingPic).into(bingPicImg);
                    }
                });
            }
        });
